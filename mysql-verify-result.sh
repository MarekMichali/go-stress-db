...
if [[ "$op_type" == "insert" ]]; then
    result=$(docker exec -i mysql bash <<'EOF'
...
EOF
    ...
    result=$(docker exec -i mysql bash <<'EOF'
    mysql -p"123456" << 'EOSQL'
    SELECT SUM(count_star) as total_executions, FORMAT_PICO_TIME(SUM(sum_timer_wait)) as total_time FROM performance_schema.events_statements_summary_by_digest WHERE digest_text LIKE 'delete%';
EOSQL
EOF
    )
else
